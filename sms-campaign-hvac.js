// sms-campaign-hvac.js — Mechanical Enterprise HVAC Textbelt Campaign
const https = require('https');
const fs = require('fs');

const CONFIG = {
  TEXTBELT_KEY: '06aa74dcb12c73154e34300053413dd8479b0cddx35TUDd3zDznHUE2qiPma7cwr',
  ...
  LIMIT: (() => { const i = process.argv.indexOf('--limit'); return i !== -1 ? parseInt(process.argv[i+1]) : null; })(),
...
function fn(name) { if (!name) return 'Homeowner'; const f = name.trim().split(' ')[0]; return f.charAt(0).toUpperCase() + f.slice(1).toLowerCase(); }
...
function getMsg(lead, i) { const msg = TEMPLATES[i % TEMPLATES.length](lead); return msg.length > 320 ? TEMPLATES[1](lead) : msg; }
...
function sendSMS(phone, message) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({ hostname: 'textbelt.com', path: '/text', method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) },
    }, res => { let buf = ''; res.on('data', c => buf += c); res.on('end', () => { try { resolve(JSON.parse(buf)); } catch { resolve({ success: false, error: buf }); } }); });
...
function sbReq(method, path, body) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({ hostname: host, path, method,
...
async function updateStatus(id, status, tbId) {
  return sbReq('PATCH', `/rest/v1/leads?id=eq.${id}`, { status, call_id: tbId || null, updated_at: new Date().toISOString() });
}
...
async function main() {
  ...
  if (!CONFIG.DRY_RUN && (quota.quotaRemaining ?? 999) < 10) { console.log('❌ Not enough credits. Top up at https://textbelt.com'); return; }
  const leads = await fetchLeads();
  console.log(`✅ ${leads.length} leads ready\n`);
  if (!leads.length) { console.log('No leads found. Run import-hvac-leads.js first.'); return; }
  if (CONFIG.DRY_RUN) {
    console.log('--- PREVIEW (first 3) ---\n');
    leads.slice(0, 3).forEach((l, i) => { const msg = getMsg(l, i); console.log(`To: ${l.phone} (${l.name})\n[${msg.length} chars]: ${msg}\n`); });
    console.log(`💳 Credits needed: ${CONFIG.LIMIT || leads.length} | Available: ${quota.quotaRemaining ?? 'unknown'}`);
    console.log('\n✅ Dry run done. Remove --dry-run to send.'); return;
  }
  const results = []; let sent = 0, failed = 0;
  for (let i = 0; i < leads.length; i++) {
    const lead = leads[i]; const msg = getMsg(lead, i);
    ...
    try {
      ...
      if (res.success) {
        ...
        results.push({ ...lead, status: 'sms-sent', tbId: res.textId }); sent++;
      } else {
        const err = res.error || 'failed'; console.log(`❌ ${err}`);
        await updateStatus(lead.id, 'sms-failed', null);
        results.push({ ...lead, status: 'sms-failed', error: err }); failed++;
        if (err.toLowerCase().includes('quota') || err.toLowerCase().includes('credit')) { console.log('\n💳 Out of credits!'); break; }
      }
    } catch(e) { console.log(`❌ ${e.message}`); results.push({ ...lead, status: 'sms-error', error: e.message }); failed++; }
