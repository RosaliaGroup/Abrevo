...
exports.handler = async (event) => {
  ...
  try {
    ...
    //
    // The hash lives in a Netlify environment variable, and a running function
    // cannot rewrite its own environment — so this GENERATES the new hash and
    // returns it to paste into Netlify. It deliberately does not offer a
    // "forgot password" flow: with credentials in env there is nowhere to send
    // a reset token to, and moving them into the database to enable that would
    // widen the blast radius of a database compromise.
    //
    // Requires the CURRENT password even though the caller already holds a
    // session, so a borrowed laptop or a stolen cookie can't change it.
    if (route === "/auth/change-password" && method === "POST") {
      const sess = requireSession(event);
      if (!sess.ok) return json(401, { ok: false, error: "unauthenticated" });
      if (!requireCsrf(event, sess.token)) return json(403, { ok: false, error: "csrf_failed" });

      let body; try { body = JSON.parse(event.body || "{}"); } catch { return json(400, { ok: false, error: "invalid_json" }); }

      const ip = auth.getClientIp(event);
      let fails = 0;
      try { fails = await recentFailures(ip); } catch { /* lockout store unreachable */ }
      if (fails >= LOCK_MAX) return json(429, { ok: false, error: "locked_out" });

      if (!auth.verifyPassword(String(body.current || ""), e.HASH)) {
        await recordAttempt(ip, e.USER, false);
        return json(401, { ok: false, error: "current_password_wrong" });
      }

      const next = String(body.next || "");
      if (next.length < 12) return json(400, { ok: false, error: "too_short" });
      if (next.length > 200) return json(400, { ok: false, error: "too_long" });
      if (!/[a-z]/i.test(next) || !/\d/.test(next)) return json(400, { ok: false, error: "needs_letter_and_number" });
      if (auth.verifyPassword(next, e.HASH)) return json(400, { ok: false, error: "same_as_current" });

      const hash = auth.hashPassword(next);
      console.log("[auth] password hash regenerated by", sess.user);
      // The hash is not a secret in the way the password is — it can't be
      // reversed — but it is returned only to an authenticated caller who has
      // just proved they know the current password.
      return json(200, { ok: true, hash });
    }
    ...
    if (route.startsWith("/leads/") && route.endsWith("/emails") && method === "GET") {
      ...
      if (!validId(id)) return json(400, { ok: false, error: "bad_id" });
      // Read the real thread. Falls back to the two legacy columns for leads
      // that predate the emails table and weren't backfilled.
      const thread = await sbGet(`emails?lead_id=eq.${encodeURIComponent(id)}&select=direction,subject,body,from_email,to_email,created_at&order=created_at.asc&limit=50`);
